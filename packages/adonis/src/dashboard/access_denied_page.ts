 ...
 * mirrors the media SPA so the refusal reads as part of the same product, not a server default.
 *
...
export interface AccessDeniedInfo {
  ...
  /** The console's mount path (`/media/dashboard`; `''` when mounted at the root). */
  basePath: string;
...
export interface AccessDeniedPageOptions {
  /** Console name in the eyebrow and `<title>`. Default: the console's own name (`Media`). */
  brand?: string;
...
export const CONSOLE = {
  brand: 'Media',
  accent: '#22d3ee',
  packageName: '@adonis-agora/media',
} as const;
