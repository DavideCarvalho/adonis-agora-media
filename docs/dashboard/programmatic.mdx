...
| `objects(disk, { prefix?, after?, first? })` | `ObjectListResponse` | One page of delimiter listing: `folders` (common prefixes) + `files`, plus the cursor envelope (`nextCursor` / `hasNext`). See [Cursor pagination](#cursor-pagination). |
...
| `collections({ collection?, ownerType?, ownerId?, prefix?, after?, first? })` | `CollectionListResponse` | Delegates straight to `MediaStore.list` — no bespoke query. [Cursor-paginated](#cursor-pagination). |
