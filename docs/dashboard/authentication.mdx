...
The console is served at a literal path (`/media/dashboard` by default), which means a plain browser navigation to it carries none of your app's identity. You have two ways to close that gap.

The first is `middleware` — front the whole console with your own AdonisJS guard. That works, and if your app already has a session cookie the browser sends on every request, it may be all you need.

The second is `auth`: a **self-contained session for the console itself**, signed by the console, carried in its own cookie, with a login screen the SPA renders when the cookie is missing. It exists because the first option covers only the case where your app's auth is already cookie-shaped and already scoped to the path the console lives at. Token auth in a header, a separate admin realm, an SPA that holds its credentials in memory — none of those survive a full-page navigation.

<Callout type="info" title="They compose">
  `auth` and `middleware` are independent gates on the same routes. Configure both and a request
  must pass both — your middleware first, the console's session guard second.
</Callout>
