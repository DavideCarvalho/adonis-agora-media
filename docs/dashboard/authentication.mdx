...
[access-denied page](#the-access-denied-page) for a **page** navigation (the SPA shell or its
...
- **A redirect the hook writes wins.** If `authorize` sets a `location` header
  ...
  leaves that redirect alone instead of overwriting it with the `403` — the
...
- **A throwing hook fails closed.** If `authorize` throws, the request is denied
  (`403`) — a buggy hook never accidentally opens the console.
