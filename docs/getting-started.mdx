...
<Callout type="info" title="The owner is yours to define">
  `ownerType` and `ownerId` are just strings the library stores and filters on — `'Post'`, `'User'`, `'org:42'`, whatever scheme you like. The library never loads your models; it only records the reference. Only `media.library.for(ownerType, ownerId)` coerces for you (it accepts `string | number`); `attach` and `list` take a **string** `ownerId`, so wrap a numeric id with `String(id)`.
</Callout>
