...
Resolve the `MediaManager` from the container and attach a file to an entity's collection:
...
await media.library.url(m.id, 'thumb')   // generated on first call, then cached
await media.library.list('Post', String(post.id), 'gallery')
await media.library.delete(m.id)         // removes the original AND every conversion
```
