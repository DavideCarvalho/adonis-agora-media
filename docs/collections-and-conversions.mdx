...
| nothing recognisable | falls back to the declared type (already whitelisted) |
...
The embedded signature table covers **PNG, JPEG, GIF, WEBP and PDF** — no extra dependency. Formats with no fixed signature (SVG, CSV, plain text, most office formats) are *unknown*, not invalid: rejecting them would break legitimate uploads, so the declared type stands. You can call the detector yourself with `detectMimeType(head)`.
