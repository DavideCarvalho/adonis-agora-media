...
<Callout type="info">
  Ordering is append-only in 0.1.0 — `attach` always takes the next slot. There's no built-in reorder API yet; if you need to resequence, update the `order` field through your own store access. (See the [Roadmap](/docs/media/roadmap).)
</Callout>
