...
and `TusUploadHandler` map that to a `400` instead of leaking a storage-layer crash. Ordinary file
names (including unicode ones) are unaffected; only path-traversal- and absolute-path-shaped names
are rejected.
